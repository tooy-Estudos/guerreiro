...
const streakIcon = s => s>=30?'👑':s>=20?'🔥':s>=10?'⚡':'🎯';
...
const DEFAULT_STATE = {
  mission:null, action:null, opState:null, tiredChip:null,
...
function loadState() {
  try {
    const raw = localStorage.getItem('cc-v1');
    if (!raw) return {...DEFAULT_STATE, lastDate:getTodayStr()};
    const saved = JSON.parse(raw);
    const today = getTodayStr();
    if (saved.lastDate === today) return {...DEFAULT_STATE, ...saved};
    const didSomething = !!(saved.flags?.acao || saved.flags?.naoZerou);
    const playedYesterday = saved.lastDate === getYesterdayStr();
    return {
      ...DEFAULT_STATE,
      streak: (playedYesterday && didSomething) ? (saved.streak||0) : 0,
      lastDate: today,
      panel: saved.panel || DEFAULT_STATE.panel,
    };
  } catch { return {...DEFAULT_STATE, lastDate:getTodayStr()}; }
}
...
function Btn({label,variant='primary',onClick,disabled,full,small}){
  const v={
    primary:{background:C.gold,color:'#0B0B0F',border:'none'},
    secondary:{background:'transparent',color:C.gold,border:`1.5px solid ${C.gold}`},
    critical:{background:C.red,color:C.text,border:'none'},
  }[variant];
...
function Toast({msg,type='success'}){
  if(!msg)return null;
  const col = type==='warn'?C.yellow:C.green;
  return <div style={{position:'fixed',top:72,left:'50%',transform:'translateX(-50%)',
    background:C.bgCard,border:`1px solid ${col}66`,borderRadius:10,padding:'10px 20px',
    zIndex:9999,display:'flex',alignItems:'center',gap:8,maxWidth:320}}>
...
function Timer90({onClose,onComplete}){
  ...
  return <div style={{position:'fixed',inset:0,background:'rgba(11,11,15,0.97)',display:'flex',
    ...
    {done
      ?<Btn label="INICIAR AÇÃO DO GUERREIRO ⚔️" onClick={()=>{onComplete();onClose();}}/>
...
function ScoreBar({flags,score}){
  const items=[
    {k:'missao',label:'Missão',pts:2},
    {k:'acao',label:'Ação',pts:3},
    {k:'fuga',label:'Fuga',pts:2},
    {k:'eus',label:'Reflexão',pts:1},
    {k:'naoZerou',label:'Mínimo',pts:2},
  ];
  return <div style={{background:C.bgCard,border:`1px solid ${C.border}`,borderRadius:14,padding:'12px 14px'}}>
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
      <div style={{fontSize:9,fontFamily:'monospace',color:C.textSub,letterSpacing:'0.15em'}}>SCORE DO DIA</div>
      <div style={{display:'flex',alignItems:'baseline',gap:4}}>
        <span style={{fontFamily:'monospace',fontSize:22,fontWeight:700,color:scoreColor(score),lineHeight:1}}>{score}</span>
        <span style={{fontSize:10,fontFamily:'monospace',color:C.textSub}}>/10 · {scoreLabel(score)}</span>
      </div>
    </div>
    <div style={{display:'flex',gap:3,marginBottom:5}}>
      {items.map(it=>(
        <div key={it.k} style={{flex:it.pts,height:4,borderRadius:2,
          background:flags[it.k]?scoreColor(score):C.bgSub,transition:'background 0.3s'}}/>
      ))}
    </div>
    <div style={{display:'flex'}}>
      {items.map(it=>(
        <div key={it.k} style={{flex:it.pts,textAlign:'center',fontSize:8,fontFamily:'monospace',
          color:flags[it.k]?C.textSub:'#374151',letterSpacing:'0.03em'}}>
          {flags[it.k]?'✓ ':''}{it.label}
        </div>
      ))}
    </div>
...
// ─── ABA HOJE ───
function TabHoje({state,setState,toast,goToTab}){
  ...
  const openModal=()=>{
    setMTitulo(state.mission?.titulo||'');
    setMProva(state.mission?.prova||'');
    setShowModal(true);
  };

  const confirmMission=()=>{
    if(!mTitulo.trim()||!mProva.trim())return;
    const isEdit=!!state.mission;
    setState(s=>({...s,
      mission:{titulo:mTitulo.trim(),prova:mProva.trim()},
      action:isEdit?s.action:{desc:`Resolver a prova: ${mProva.trim()}`,status:'pendente'},
      flags:{...s.flags,missao:true},
    }));
    setShowModal(false);setMTitulo('');setMProva('');
    toast(isEdit?'Missão atualizada.':'Missão definida. +2 pontos.');
  };

  return <div style={{padding:'12px 16px 100px',display:'flex',flexDirection:'column',gap:8}}>
    ...
    {showModal&&<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.7)',display:'flex',
      ...
      <div onClick={e=>e.stopPropagation()} style={{background:C.bgCard,borderRadius:'20px 20px 0 0',
        ...
        <input autoFocus value={mTitulo} onChange={e=>setMTitulo(e.target.value)}
          onKeyDown={e=>{if(e.key==='Enter'&&mTitulo.trim())provaRef.current?.focus();}}
          placeholder="Aprovação OAB..."
          style={{width:'100%',background:C.bgSub,border:`1px solid ${C.border}`,borderRadius:10,
            padding:'12px 14px',color:C.text,fontSize:14,outline:'none',boxSizing:'border-box',marginBottom:14}}/>
        ...
        <input ref={provaRef} value={mProva} onChange={e=>setMProva(e.target.value)}
          onKeyDown={e=>{if(e.key==='Enter')confirmMission();}}
          placeholder="Resolver 10 questões..."
          style={{width:'100%',background:C.bgSub,border:`1px solid ${C.border}`,borderRadius:10,
            padding:'12px 14px',color:C.text,fontSize:14,outline:'none',boxSizing:'border-box',marginBottom:24}}/>
        <Btn label="CONFIRMAR MISSÃO ✓" full disabled={!mTitulo.trim()||!mProva.trim()} onClick={confirmMission}/>
      </div>
    </div>}

    {/* SCORE BAR — sempre visível */}
    <ScoreBar flags={f} score={score}/>
    ...
    {state.mission
      ?<div style={{background:'linear-gradient(135deg,#101828,#0D1F35)',border:`1.5px solid ${C.gold}`,
          ...
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
            ...
            <button onClick={openModal} style={{background:'none',border:'none',color:C.textSub,
              fontSize:10,fontFamily:'monospace',cursor:'pointer',letterSpacing:'0.05em',
              padding:'0 0 8px 12px',flexShrink:0}}>editar</button>
          </div>
      ...
      :<div onClick={openModal} style={{background:C.bgCard,border:`1.5px dashed ${C.gold}4D`,
          borderRadius:16,padding:'14px 16px',display:'flex',flexDirection:'column',
          justifyContent:'center',cursor:'pointer',minHeight:96}}>
          <Label text="⚡ MISSÃO CRÍTICA" color={C.gold}/>
    ...
    <div style={{background:C.bgCard,border:`1px solid ${C.border}`,borderRadius:16,padding:'14px 16px'}}>
      ...
      {!state.mission
        ?<>
            <div style={{fontSize:13,color:C.textSub,marginBottom:12}}>Defina sua missão primeiro para desbloquear a ação.</div>
            <Btn label="⚡ EXECUTAR AGORA" full disabled/>
          </>
        :<>
            <div style={{fontSize:14,fontWeight:600,color:C.text,marginBottom:4,lineHeight:1.4}}>{state.action?.desc}</div>
            <div style={{fontSize:11,color:C.textSub,fontFamily:'monospace',marginBottom:12}}>⏱ 25 min focado</div>
            {state.action?.status==='pendente'&&
            ...
            {state.action?.status==='exec'&&<div style={{display:'flex',flexDirection:'column',gap:8}}>
              <div style={{padding:'9px 12px',background:'rgba(201,169,106,0.08)',
                border:'1px solid rgba(201,169,106,0.25)',borderRadius:8,
                fontSize:11,color:C.gold,fontFamily:'monospace'}}>⏳ Executando...</div>
              <Btn label="✓ MARCAR COMO CONCLUÍDA" variant="secondary" full onClick={()=>{
                setState(s=>({...s,action:{...s.action,status:'done'},
            ...
            {state.action?.status==='done'&&
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                ...
                <button onClick={()=>goToTab('identidade')} style={{background:'none',border:'none',
                  color:C.gold,fontSize:11,fontFamily:'monospace',cursor:'pointer',letterSpacing:'0.04em'}}>
                  ir para reflexão →
                </button>
              </div>}
          </>}
    ...
    {!actionDone&&pendingEscape&&<div style={{background:'rgba(245,158,11,0.07)',
      ...
      <div>
        <div style={{fontSize:9,fontFamily:'monospace',color:C.yellow,letterSpacing:'0.15em',marginBottom:3}}>RISCO PRINCIPAL</div>
        <div style={{fontSize:13,color:C.text,fontWeight:600}}>{pendingEscape.label}</div>
      </div>
      <button onClick={()=>{setEscapeOpen(true);setActiveEscape(pendingEscape.id);}}
        style={{background:'none',border:`1px solid ${C.yellow}44`,borderRadius:7,
          padding:'5px 10px',color:C.yellow,fontFamily:'monospace',fontSize:10,
        ...
        {state.tiredChip&&<button
          onClick={()=>{
            setState(s=>({...s,
              ...
              streak:s.flags.acao?s.streak:s.streak+1,
              opState:null,tiredChip:null,
            }));
            toast('Dia protegido. Corrente mantida.');
          }}
    ...
    {/* 5. STREAK — visível após qualquer conclusão */}
    {(actionDone||state.flags.naoZerou)&&<div style={{background:C.bgCard,
      border:`1px solid ${C.border}`,borderRadius:14,padding:'11px 16px',
      display:'flex',justifyContent:'space-between',alignItems:'center'}}>
      <div>
        <div style={{fontSize:9,fontFamily:'monospace',color:C.textSub,letterSpacing:'0.1em',marginBottom:2}}>CORRENTE SEM ZERAR</div>
        <div style={{display:'flex',alignItems:'baseline',gap:4}}>
          <span style={{fontFamily:'monospace',fontSize:28,fontWeight:700,color:C.gold,lineHeight:1}}>{state.streak}</span>
          <span style={{fontSize:12,color:C.textSub,fontFamily:'monospace'}}>/30 dias</span>
        </div>
      </div>
      <span style={{fontSize:28}}>{streakIcon(state.streak)}</span>
    </div>}
...
// ─── ABA IDENTIDADE ───
function TabIdentidade({state,setState,toast,goToTab}){
  ...
  return <div style={{padding:'20px 16px 100px',display:'flex',flexDirection:'column',gap:14}}>
    ...
    {showConfrontar&&<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.75)',display:'flex',
      ...
      <div onClick={ev=>ev.stopPropagation()} style={{background:C.bgCard,borderRadius:'20px 20px 0 0',
        ...
        <div style={{fontSize:22,fontFamily:'Georgia,serif',color:C.text,lineHeight:1.3,marginBottom:20}}>{artig.principio}</div>
        <div style={{fontSize:10,fontFamily:'monospace',color:C.textSub,letterSpacing:'0.15em',marginBottom:6}}>CONFRONTO</div>
        <div style={{fontSize:14,color:C.text,fontStyle:'italic',marginBottom:20,lineHeight:1.5}}>"{artig.pergunta}"</div>
    ...
    <div style={{background:'linear-gradient(135deg,#0D0D0D,#1A1206)',
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        <button onClick={()=>setWarIdx(i=>(i-1+WAR_CARDS.length)%WAR_CARDS.length)}
          style={{background:'transparent',border:`1px solid ${C.border}`,borderRadius:8,
            padding:'8px 16px',color:C.textSub,fontFamily:'monospace',fontSize:12,cursor:'pointer'}}>
          ← anterior
        </button>
        <div style={{display:'flex',gap:4}}>
          {WAR_CARDS.map((_,i)=><div key={i} onClick={()=>setWarIdx(i)}
            style={{width:6,height:6,borderRadius:'50%',cursor:'pointer',
              background:i===warIdx?C.gold:C.border}}/>)}
        </div>
        <button onClick={()=>setWarIdx(i=>(i+1)%WAR_CARDS.length)}
          style={{background:'transparent',border:`1px solid ${C.border}`,borderRadius:8,
            padding:'8px 16px',color:C.textSub,fontFamily:'monospace',fontSize:12,cursor:'pointer'}}>
          próximo →
        </button>
    ...
    <div style={{background:C.bgCard,border:`1px solid ${C.border}`,borderRadius:20,padding:20}}>
      ...
      <div style={{display:'flex',gap:10,marginBottom:state.eusSaved?10:0}}>
        <div style={{flex:1}}>
        ...
        <div style={{flex:1}}>
          <Btn label="ATIVAR GUERREIRO ⚔️" full disabled={!e.presente.trim()}
            onClick={()=>{
              if(!e.presente.trim())return;
              if(!state.mission){toast('Defina uma missão antes de ativar o Guerreiro.','warn');return;}
              setState(s=>({...s,action:{
                desc:e.presente.trim(),
                status:s.action?.status==='done'?'done':'pendente',
              }}));
              toast('Ação do Guerreiro atualizada.');
            }}/>
        </div>
      </div>
      {state.eusSaved&&<button onClick={()=>goToTab('hoje')}
        style={{width:'100%',padding:'10px',background:'transparent',
          border:`1px solid ${C.gold}44`,borderRadius:10,color:C.gold,
          fontFamily:'monospace',fontSize:11,cursor:'pointer',letterSpacing:'0.05em'}}>
        ← voltar para HOJE e executar
      </button>}
    </div>
    ...
    <div style={{background:C.bgCard,border:`1.5px solid ${C.gold}44`,borderRadius:20,padding:20}}>
      ...
      <div style={{fontSize:20,fontFamily:'Georgia,serif',color:C.text,lineHeight:1.35,marginBottom:20}}>{artig.principio}</div>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
        <button onClick={()=>setArtIdx(i=>(i-1+CONSTITUTION.length)%CONSTITUTION.length)}
          style={{background:'transparent',border:`1px solid ${C.border}`,borderRadius:8,
            padding:'8px 16px',color:C.textSub,fontFamily:'monospace',fontSize:12,cursor:'pointer'}}>←</button>
        <div style={{display:'flex',gap:4}}>
          {CONSTITUTION.map((_,i)=><div key={i} onClick={()=>setArtIdx(i)}
            style={{width:5,height:5,borderRadius:'50%',cursor:'pointer',
              background:i===artIdx?C.gold:C.border}}/>)}
        </div>
        <button onClick={()=>setArtIdx(i=>(i+1)%CONSTITUTION.length)}
          style={{background:'transparent',border:`1px solid ${C.border}`,borderRadius:8,
            padding:'8px 16px',color:C.textSub,fontFamily:'monospace',fontSize:12,cursor:'pointer'}}>→</button>
      </div>
...
function TabEvolucao({state,setState,toast}){
  ...
  return <div style={{padding:'20px 16px 100px',display:'flex',flexDirection:'column',gap:12}}>
    ...
    {corr
      ?<div style={{background:state.corrDone?'rgba(34,197,94,0.06)':`linear-gradient(135deg,${corr.cor}18,${corr.cor}08)`,
          border:`1.5px solid ${state.corrDone?C.green+'44':corr.cor+'55'}`,borderRadius:20,padding:20}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14}}>
          ...
          {state.corrDone
            ...
            :<>
                <div style={{fontSize:22,fontWeight:700,color:C.text,lineHeight:1.2,marginBottom:4}}>{corr.icon} {corr.area}</div>
                <div style={{fontSize:14,color:C.text,lineHeight:1.5,marginBottom:20}}>{corr.acao}</div>
    ...
    <div style={{background:C.bgCard,border:`1px solid ${hasData?convColor+'44':C.border}`,borderRadius:16,padding:18}}>
      ...
      {!hasData
        ?<div style={{fontSize:13,color:C.textSub,marginBottom:14}}>Registre um conteúdo consumido para começar.</div>
        :<>
            ...
            <div style={{height:6,background:C.bgSub,borderRadius:3,marginBottom:12,overflow:'hidden'}}>
              <div style={{height:'100%',width:`${Math.min(pct,100)}%`,background:convColor,
      ...
      <div style={{display:'flex',gap:10}}>
        <button disabled={c.consumidos===0}
          onClick={()=>{
            if(c.consumidos===0)return;
        ...
        <button disabled={blocked}
          onClick={()=>{
            if(blocked)return;
            setState(s=>({...s,corrDone:false,conversion:{...s.conversion,consumidos:s.conversion.consumidos+1}}));
          }}
          style={{flex:1,padding:'11px 0',background:'transparent',
            border:`1px solid ${blocked?C.red+'44':C.border}`,borderRadius:12,
            color:blocked?C.red:C.textSub,fontFamily:'monospace',fontSize:12,
            cursor:blocked?'default':'pointer',opacity:blocked?0.4:1}}>
          {blocked?'bloqueado':'+ Conteúdo'}
        </button>
      </div>
    </div>
    ...
    <div style={{background:C.bgCard,border:`1px solid ${C.border}`,borderRadius:16,overflow:'hidden'}}>
      <button onClick={()=>setPanelOpen(o=>!o)} style={{width:'100%',padding:'16px 18px',
        ...
        <div style={{textAlign:'left'}}>
          <div style={{fontSize:10,fontFamily:'monospace',color:C.textSub,letterSpacing:'0.2em',marginBottom:3}}>
            ⚖️ PAINEL DA VIDA
          </div>
          <div style={{fontSize:13,color:panelSummaryColor,fontWeight:600}}>{panelSummary}</div>
        </div>
        <span style={{fontSize:12,fontFamily:'monospace',color:C.gold,marginLeft:12}}>{panelOpen?'▲':'▼'}</span>
      </button>
        ...
        {PANEL_AREAS.map(a=>{
          ...
          return <div key={a.key}>
            <button onClick={()=>toggleArea(a.key)} style={{width:'100%',padding:'12px 18px',
              display:'flex',justifyContent:'space-between',alignItems:'center',
              background:isActive?`${lvColor(lv)}0D`:'transparent',border:'none',cursor:'pointer'}}>
              <div style={{display:'flex',alignItems:'center',gap:8}}>
              ...
              <div style={{display:'flex',alignItems:'center',gap:6}}>
                ...
                <span style={{fontSize:10,fontFamily:'monospace',color:lvColor(lv),letterSpacing:'0.04em'}}>{lvLabel(lv)}</span>
                <span style={{fontSize:10,color:C.textSub,marginLeft:4}}>{isActive?'▲':'▼'}</span>
              </div>
            </button>
            {isActive&&<div style={{padding:'4px 18px 14px',display:'flex',flexDirection:'column',gap:8}}>
              <div style={{fontSize:11,color:C.textSub,fontFamily:'monospace',letterSpacing:'0.04em'}}>
                Micro-ação: {a.micro}
              </div>
...
export default function App(){
  ...
  const toast=(m,type='success')=>{setToastMsg(m);setToastType(type);setTimeout(()=>setToastMsg(null),2200);};

  const [state,setState]=useState(()=>loadState());

  useEffect(()=>{
    try{localStorage.setItem('cc-v1',JSON.stringify({...state,lastDate:state.lastDate||getTodayStr()}));}
    catch{}
  },[state]);

  const goToTab=(id)=>{
    if(id!=='hoje'&&state.opState==='trav')setState(s=>({...s,opState:null}));
    setTab(id);
  };
  ...
  const f=state.flags;
  const score=(f.missao?2:0)+(f.acao?3:0)+(f.fuga?2:0)+(f.eus?1:0)+(f.naoZerou?2:0);
  const pendingEscapes=ESCAPES.filter(e=>!state.fixedEscapes.includes(e.id)).length;
  ...
  return <div style={{fontFamily:'system-ui,sans-serif',background:C.bg,minHeight:'100vh',
    ...
    <div style={{position:'sticky',top:0,zIndex:200,background:C.bg,borderBottom:`1px solid ${C.border}`,padding:'12px 20px'}}>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        ...
        {score>0&&<div style={{fontFamily:'monospace',fontSize:14,fontWeight:700,color:scoreColor(score)}}>{score}/10</div>}
      </div>
    </div>

    {/* CONTENT */}
    <div style={{flex:1,overflowY:'auto'}}>
      {tab==='hoje'&&<TabHoje state={state} setState={setState} toast={toast} goToTab={goToTab}/>}
      {tab==='identidade'&&<TabIdentidade state={state} setState={setState} toast={toast} goToTab={goToTab}/>}
    ...
    <div style={{position:'fixed',bottom:0,left:'50%',transform:'translateX(-50%)',width:'100%',maxWidth:480,
      ...
      {tabs.map(t=>{
        const a=tab===t.id;
        const badge=t.id==='hoje'&&pendingEscapes>0&&!a;
        return <button key={t.id} onClick={()=>goToTab(t.id)}
          style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:3,
            background:'none',border:'none',cursor:'pointer',padding:'8px 4px',position:'relative'}}>
          <span style={{fontSize:a?22:18}}>{t.icon}</span>
          {badge&&<div style={{position:'absolute',top:6,right:'calc(50% - 16px)',
